...
manipulates 2D points and prints the number 3.
...
- Integer literals are either 0 or start with an optional minus (-) sign, followed by a non-zero digit and then followed by zero or more digits. -0 is not an integer literal.
...
- Literals: `-2147483648, ..., -1, 0, 1, ..., 2147483647` (integer literals are 32 bits in two's complement), `true`, `false`, `Unit`, in addition to quote-enclosed string literals
- Binary operators: `;` `+` `-` `*` `/` `%` `^` `==` `!=` `<` `<=` `>` `>=` `!` `||` `&&` `.`
- Unary operators: `-` `!`
...
`2 ^ - 3 ^ 4 * 5` is to be understood as `(2 ^ ((- 3) ^ 4)) * 5`, where - 3 is the unary minus operator applied to the (positive) integer value 3
- The sequencing operator, `;`, associates to the right and binds looser than all other operators. For example, `1 ; 2 || 3 ; 4` is to be understood
...
- The precedence of `while` loops and conditionals (both `if-then-else` and `if-then`) sits between that of `;` and `||`. For example, 
...
Records introduce a notion of subtyping. We use notation `type1 <: type2` to say that `type1` is a subtype of `type2`. A record with more fields is a subtype of a record with fewer fields. 
This is known as width subtyping (think of methods in an object-oriented language). For example:
...
each `expi` has type that is a subtype of `typei`
- Variable declaration `var id : type = exp` is of type `Unit` if `exp` is of a that is subtype of `type`. Furthermore, if the declaration appears at the left 
...
of the conditional is the value of `exp2` or `exp3`
